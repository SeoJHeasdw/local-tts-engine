...
import {
  ...
  voiceFindingKey,
  withClearedFindings,
} from "../../electron-app/shared/index.mjs";
import { voiceFindingReason } from "../../electron-app/renderer/view-utils.mjs";
...
test("검수 페이지는 연속 스텝의 원문과 시각을 합친다", async () => {
  const { reviewPages } = await import('../../electron-app/shared/index.mjs');
...
test("구간 무음은 유효한 짧은 범위만 허용한다", async () => {
  const { muteRegionFilter } = await import('../../electron-app/shared/index.mjs');
...
test('짧은 교체 음성이 선택 구간보다 길면 잘라내지 않고 거절한다', async () => {
  const {replaceRegionPlan}=await import('../../electron-app/shared/index.mjs');
