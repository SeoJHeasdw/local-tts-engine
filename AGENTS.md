...
- **결과물 품질을 속도·자원 절약보다 우선한다.** 사용자는 M4 Max 36GB에서
  품질에 기여하는 검수·재시도·정밀 처리 비용을 감수한다. 자원 절약만을 이유로
  모델 품질·출력 품질·검수 범위·재시도 예산을 낮추지 않는다.
- 최적화는 중복 조회·불필요한 재인코딩·실패 뒤 남은 작업 실행처럼 결과 품질에
  기여하지 않는 낭비를 먼저 줄인다. 품질과 성능의 교환이 필요한 변경은 효과와
  손실을 설명하고 사용자 판단을 받는다.
