...
- 자막 원문은 유지하면서 TTS 입력만 구조적으로 정규화한다. 현재 사용자 승인
  ...
  `Qwen3.6-27B → 큐웬 삼 점 육 이십칠 비`, `50개 → 오십 개`다.
...
- 발음문이 달라졌거나 미해결 영문·숫자가 남은 청크는 최대 3개 시드 후보를
  만들고 Whisper 받아쓰기와 파형 검사로 자동 선택한다. 모두 실패하면 가장
  나은 후보를 보존하고 정확한 페이지를 `needsReview`에 기록한다.
- 앱의 최근 결과에서 실패 페이지를 바로 볼 수 있다. 영상 편집에서 그 페이지
  범위의 목소리를 다시 만들면 기존 영상 전체 음성을 덮지 않고 해당 구간만
  교체하며, 이후 타임라인을 길이 차이만큼 이동한다.
- 제작 음색과 LoRA 강도 0.60은 그대로다.
