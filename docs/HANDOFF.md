...
기본 자산 위치는 `data/private/voice/`, `artifacts/production/captions/`,
`artifacts/production/videos/`다. `udemy-agent`에는 강의 대본·슬라이드 UI와
캡처 실행 코드만 남는다.
