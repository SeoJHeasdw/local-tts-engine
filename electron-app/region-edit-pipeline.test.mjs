...
import {regionReplacementPlan,retimeCaptions,captionText,editedReviewContext} from './review-media.mjs';
import {patchedTimeline,videoTimelineFileName,summarizeChecks,muteRegionFilter} from './pipeline-utils.mjs';

const source=await fs.readFile(new URL('./main.mjs',import.meta.url),'utf8');
const methods=source.slice(source.indexOf('async function runMuteEdit('),source.indexOf('async function runVideoEdit('));
...
test('앱의 실제 구간 저장 경로가 새 영상·타임라인·자막·검수 기록을 함께 만든다',async()=>{
  ...
  try{
    ...
    const context=vm.createContext({fs,path,regionReplacementPlan,retimeCaptions,captionText,editedReviewContext,
      patchedTimeline,videoTimelineFileName,summarizeChecks,muteRegionFilter,
    ...
    vm.runInContext(methods+';globalThis.replace=runRegionReplaceEdit;globalThis.mute=runMuteEdit;',context);
    const out=path.join(directory,'edit');await fs.mkdir(out);
    const result=await context.replace({videoToken:'video',audioToken:'audio',name:'repair',muteStart:1,muteEnd:1.6,durationPolicy:'match-audio'},out);
    ...
    const muted=await context.mute({videoToken:'video',name:'quiet',muteStart:1.125,muteEnd:1.175},removed);
